...
class ProductAPIView(views.APIView):
    ...
    def post(self, request):
        try:
            ...
            product = Product(
                name=data['name'], unit=data['unit'], price=data['price'],
                discount=data['discount'], amount=data['amount'], thumbnail=data['thumbnail'],
...
class ProductImageAPIView(views.APIView):
    def get(self, request, product_id_slug):
        try:
            ...
            return custom_response('Get all product images successfully!', 'Success', serializer.data, 200)
        except:
    ...
    def post(self, request, product_id_slug):
        try:
            ...
            product_image = ProductImage(product_id=product, image_url=data['image_url'])
            product_image.save()
    ...
    def post(self, request, product_id_slug):
        try:
            ...
            product_comment = ProductComment(
                product_id=product, rating=data['rating'], comment=data['comment'],
                user_id=user, parent_id=data.get('parent_id')
            )
